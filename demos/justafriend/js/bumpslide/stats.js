define([], function(){
    ...
    m.show = function (styles) {
        if(m._visible) return;
        clearInterval( m._interval )
        m._visible = true;
        if(m.stats!=null) {
            m._interval = setInterval(m.stats.update, 1000 / 60);
            $('body').append( m._element );
            return;
        }
        if(styles==null) styles = {position:'fixed',top:5, right:5};


        //require( ['https://github.com/mrdoob/stats.js/raw/master/build/Stats.js'], function () {
                m.stats = new Stats();
                m._interval = setInterval(m.stats.update, 1000 / 60);
                m._element = $(m.stats.domElement).css( styles );
                $('body').append( m._element );
        //    });
    }
