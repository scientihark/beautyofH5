...
';
function Renderer() {
this.tileTexture = Texture.fromImage(document.getElementById('tiles'), {
minFilter: gl.LINEAR_MIPMAP_LINEAR,
wrap: gl.REPEAT,
format: gl.RGB
});
this.lightDir = new Vector(2.0, 2.0, -1.0).unit();
this.causticTex = new Texture(1024, 1024);
this.waterMesh = Mesh.plane(200, 200, { coords: false, normals: false });
this.waterShaders = [];
for (var i = 0; i < 2; i++) {
...
this.sphereMesh = Mesh.sphere(10, { coords: false, normals: false });
this.sphereShader = new Shader(helperFunctions + '\
varying vec3 position;\
void main() {\
position = sphereCenter + gl_Vertex * sphereRadius;\
gl_Position = gl_ModelViewProjectionMatrix * vec4(position, 1.0);\
}\
', helperFunctions + '\
varying vec3 position;\
...
');
this.cubeMesh = Mesh.cube({ coords: false, normals: false });
this.cubeMesh.triangles.splice(4, 2);
this.cubeMesh.compile();
this.cubeShader = new Shader(helperFunctions + '\
varying vec3 position;\
void main() {\
position = gl_Vertex;\
...
this.causticsShader = new Shader(helperFunctions + '\
varying vec3 oldPos;\
varying vec3 newPos;\
varying vec3 ray;\
\
